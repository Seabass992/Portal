...
namespace Portal.Helpers
{
    public class Hashing
    {
        ...
        public static bool CheckPassword(string submitted, string storedHash)
        {
            ...
            if (hashBytes.Length != 32+20) throw new Exception("decoded stored hash is not correct length");
            var salt = hashBytes.Skip(32).ToArray();
            if(salt.Length != 20) throw new Exception("decoded salt length is not correct length");
